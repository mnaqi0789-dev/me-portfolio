          ...
          {stackGroups.map((group, i) => (
            <Reveal key={group.label} delay={i * 0.05}>
              <div className="h-full rounded-lg border border-border bg-surface p-5">
                <h3 className="font-mono text-xs tracking-[0.14em] text-text-faint uppercase">
                  {group.label}
                </h3>
                <ul className="mt-4 flex flex-wrap gap-2">
                  {group.items.map((item) => (
                    <li
                      key={item}
                      className="rounded border border-border px-2 py-1 font-mono text-[0.7rem] break-words text-text-muted transition-colors hover:border-accent hover:text-accent"
                    >
                      {item}
                    </li>
                  ))}
